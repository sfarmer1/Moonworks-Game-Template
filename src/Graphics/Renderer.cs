...
public class Renderer : MoonTools.ECS.Renderer
{
	...
	public Renderer(World         world, GraphicsDevice graphicsDevice, TitleStorage titleStorage,
					TextureFormat swapchainFormat) : base(world)
	{
		_graphicsDevice = graphicsDevice;

		_rectangleFilter = FilterBuilder.Include<Rectangle>()
										.Include<Position>()
										.Include<DrawAsRectangle>()
										.Build();
		_textFilter = FilterBuilder.Include<Text>()
								   .Include<Position>()
								   .Build();
		_spriteAnimationFilter = FilterBuilder.Include<SpriteAnimation>()
											  .Include<Position>()
											  .Build();

		_renderTexture = Texture.Create2D(_graphicsDevice, "Render Texture", GameDimensions.WIDTH,
	...
	public void Render(Window window)
	{
		...
		if (swapchainTexture != null)
		{
			...
			foreach (var entity in _rectangleFilter.Entities)
			{
				...
				var orientation = Has<Orientation>(entity)
					? Get<Orientation>(entity)
						.Angle
					: 0.0f;
				var color = Has<ColorBlend>(entity)
					? Get<ColorBlend>(entity)
						.Color
					: Color.White;
				var depth = -2f;
				if (Has<Depth>(entity))
					depth = -Get<Depth>(entity)
						.Value;
				...
				_artSpriteBatch.Add(new Vector3(position.X + rectangle.X, position.Y + rectangle.Y, depth), orientation,
									new Vector2(rectangle.Width, rectangle.Height), color, sprite.UV.LeftTop,
			...
			foreach (var entity in _spriteAnimationFilter.Entities)
			{
				...
				_artSpriteBatch.Add(new Vector3(position.X + offset.X, position.Y + offset.Y, depth), 0,
									new Vector2(sprite.SliceRect.W, sprite.SliceRect.H) * scale, color,
									sprite.UV.LeftTop,
									sprite.UV.Dimensions);
			}
