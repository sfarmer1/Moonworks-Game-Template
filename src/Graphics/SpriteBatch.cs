...
public class SpriteBatch
{
	...
	public SpriteBatch(GraphicsDevice graphicsDevice,      MoonWorks.Storage.TitleStorage titleStorage,
					   TextureFormat  renderTextureFormat, TextureFormat?                 depthTextureFormat = null)
	{
		...
		_computePipeline = ShaderCross.Create(_graphicsDevice, titleStorage,
											  System.IO.Path.Combine(shaderContentPath, "SpriteBatch.comp.hlsl.spv"),
											  "main",
											  ShaderCross.ShaderFormat.SPIRV);

		var vertShader = ShaderCross.Create(_graphicsDevice, titleStorage,
											System.IO.Path.Combine(shaderContentPath, "SpriteBatch.vert.hlsl.spv"),
											"main",
											ShaderCross.ShaderFormat.SPIRV, ShaderStage.Vertex);
		var fragShader = ShaderCross.Create(_graphicsDevice, titleStorage,
											System.IO.Path.Combine(shaderContentPath, "SpriteBatch.frag.hlsl.spv"),
											"main",
		...
		_instanceTransferBuffer = TransferBuffer.Create<SpriteInstanceData>(graphicsDevice,
																			"SpriteBatch InstanceTransferBuffer",
																			TransferBufferUsage.Upload,
																			MAX_SPRITE_COUNT);

		_instanceBuffer = Buffer.Create<SpriteInstanceData>(graphicsDevice,
															BufferUsageFlags.Vertex
															| BufferUsageFlags.ComputeStorageRead, MAX_SPRITE_COUNT);
		_instanceIndex = 0;
	...
	public void Upload(CommandBuffer commandBuffer)
	{
		...
		if (InstanceCount > 0)
		{
			...
			copyPass.UploadToBuffer(new TransferBufferLocation(_instanceTransferBuffer),
									new BufferRegion(_instanceBuffer, 0,
